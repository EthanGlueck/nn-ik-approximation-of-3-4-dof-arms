...
from forward_kinematics_3_dof import fk_3dof_batch
...
def compare_single_sample():
    ...
    test_joints = np.array([0.0, 0.0, 0.0])  # All joints at zero
    
    # Your FK result
    joints_tensor = torch.tensor(test_joints).unsqueeze(0)
    your_fk = fk_3dof_batch(joints_tensor).squeeze(0).numpy()
    
    # PyBullet FK result - GET THE TIP, NOT JUST LINK FRAME
    physicsClient = p.connect(p.DIRECT)
    p.setAdditionalSearchPath(pybullet_data.getDataPath())
    robotId = p.loadURDF("3_dof_visual_arm.urdf", basePosition=[0, 0, 0], useFixedBase=True)
    
    # Set joints to test angles
    for i, angle in enumerate(test_joints):
        p.resetJointState(robotId, i, angle)
    
    # ✅ MUCH SIMPLER: Just get the tip link directly
    linkState = p.getLinkState(robotId, 3, computeForwardKinematics=1)  # Link 3 is ee_tip
    pybullet_fk = np.array(linkState[4])  # No offset needed anymore!
    ...
    # Test with shoulder = 90° (same fix)
    test_joints2 = np.array([0.0, 1.57, 0.0])  # Shoulder at 90°
    
    joints_tensor2 = torch.tensor(test_joints2).unsqueeze(0)
    your_fk2 = fk_3dof_batch(joints_tensor2).squeeze(0).numpy()
    ...
    # ✅ Same fix for 90° test
    linkState2 = p.getLinkState(robotId, 3, computeForwardKinematics=1)
...
# Load your dataset
data = np.load("ik_dataset_3dof.npz")
...
Y_radians = Y * np.array([3.14, 1.57, 2.0])  # Convert from [-1,1] to radians

# Denormalize PyBullet positions to meters
X_meters = X * np.array([0.37, 0.37, 0.5])  # Convert from [-1,1] to meters

# Use your actual FK function (first 10k samples for speed)
Y_tensor = torch.tensor(Y_radians[:10000], dtype=torch.float32)
fk_from_gt = fk_3dof_batch(Y_tensor).numpy()   # Use your FK file
