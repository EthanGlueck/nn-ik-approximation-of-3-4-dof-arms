...
def main():
    ...
    # Load 4DOF dataset (already normalized)
    ...
    # Define the network for 4DOF (similar to 3DOF but output 4 joints)
    ...
    for epoch in range(num_epochs):
        ...
        for X_batch, Y_batch in train_loader:
            ...
            Y_pred_radians = torch.stack([
                ...
                Y_pred[:, 3] * 2.0    # Wrist: [-1,1] → [-2.0, 2.0]
            ], dim=1)
            ...
            X_batch_meters = torch.stack([
                X_batch[:, 0] * 0.94,  # X: [-1,1] 
                X_batch[:, 1] * 0.94,  # Y: [-1,1]   
                X_batch[:, 2] * 0.6   # Z: [-1,1] → [0, 0.6] (4DOF has longer reach)
            ], dim=1)
    ...
    loss_csv_path = os.path.join(BASE_DIR, "models", "data", "training_loss_4dof.csv")
    ...
    print(" Position-based 4DOF Model saved!")
