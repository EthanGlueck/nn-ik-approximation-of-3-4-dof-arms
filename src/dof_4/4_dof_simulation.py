...
joint_angles = np.zeros((TARGET_DATASET_SIZE, 4), dtype=np.float32)  # Changed from 5 to 4
dataset_index = 0
...
def generate_and_store_pose(robotId, numJoints, index):
    ...
    # Updated for 4 DOF - removed the 5th joint
    joint_angles[index] = [
        joint_values[0] / 3.14,   # Base joint (waist)
        joint_values[1] / 1.57,   # Shoulder joint
        joint_values[2] / 2.0,   # Elbow joint
        joint_values[3] / 2.0    # Wrist joint
    ]
...
def save_dataset(current_size):
    ...
    dataset_path = os.path.join(BASE_DIR, "models", "data", "ik_dataset_4dof.npz")
...
# Initialize simulation - changed to 4 joints
...
if GENERATE_DATASET:
    ...
    print(f"🤖 Starting automatic 4DOF dataset generation ({TARGET_DATASET_SIZE} samples)...")
    ...
    try:
        ...
        print("✅ 4DOF Dataset generation complete!")
    ...
    # Store current joint positions - updated for 4 DOF
    current_joint_positions = [0.0] * numJoints
    
    # Create GUI sliders for 4 joints
    ...
    joint4Slider = p.addUserDebugParameter("Wrist (X)", -2.0, 2.0, 0)  # ✅ Changed from (Y) to (X)

    sliders = [joint1Slider, joint2Slider, joint3Slider, joint4Slider]  # 4 sliders
    ...
    try:
        while True:
            ...
            if currentRestartButtonValue != lastRestartButtonValue:
                print("Restarting 4DOF simulation...")
                ...
                joint2Slider = p.addUserDebugParameter("Shoulder (X)", -1.57, 1.57, 0)
                joint3Slider = p.addUserDebugParameter("Elbow (X)", -2.0, 2.0, 0)
                joint4Slider = p.addUserDebugParameter("Wrist (X)", -2.0, 2.0, 0)
                sliders = [joint1Slider, joint2Slider, joint3Slider, joint4Slider]
                # Reset counters
                printCounter = 0
                print("4DOF simulation restarted - all joints and sliders reset to home position")
